...
static void draw_needle_cluster(VirtualBitmap *bmp, float x, float y, float length, float angle, float density, float offset_x) {
...
static void draw_spruce_tier(VirtualBitmap *bmp, float x, float y, float radius, float droop, float offset_x) {
...
static void generate_tree(Tree *t, float base_x, float base_y, int height, uint32_t *seed) {
...
static void draw_pine_tree(Tree *t, VirtualBitmap *bmp, float offset_x) {
    void draw_node_recursive(Node *n, VirtualBitmap *b, float ox, int depth, int total_segments) {
        int x1 = (int)(n->x - ox);
        int y1 = (int)n->y;
        ...
        for (int i = 0; i < n->child_count; i++) {
            ...
            draw_node_recursive(n->children[i], b, ox, is_main_trunk ? depth + 1 : depth, total_segments);
        }
    }
...
static void free_node_recursive(Node *n) {
    for (int i = 0; i < n->child_count; i++) free_node_recursive(n->children[i]);
    if (n->children) free(n->children);
    free(n);
}

/* Chunk initializer */
static void init_chunk(Chunk *c, float world_x, int layer, int height, uint32_t master_seed) {
    c->world_x = world_x;
    c->active = 1;
    uint32_t chunk_seed = master_seed ^ ((uint32_t)world_x * 0x1F351) ^ ((uint32_t)layer * 0x8421);
    if (chunk_seed == 0) chunk_seed = 0xDEADC0DE;
...
static void free_chunk(Chunk *c) {
    if (!c->active) return;
    for (int i = 0; i < c->tree_count; i++) free_node_recursive(c->trees[i].root);
    free(c->trees);
    c->active = 0;
}
...
Forest* create_forest(int width, int height) {
    ...
    f->width = width; f->height = height; f->camera_x = 0; f->chunk_count = CHUNK_POOL_SIZE;
    f->seed = (uint32_t)time(NULL); if (f->seed == 0) f->seed = 0xACE2026;
    f->chunks = (Chunk**)malloc(sizeof(Chunk*) * LAYER_COUNT);
    for (int l = 0; l < LAYER_COUNT; l++) {
        f->chunks[l] = (Chunk*)calloc(CHUNK_POOL_SIZE, sizeof(Chunk));
        for (int i = 0; i < CHUNK_POOL_SIZE; i++) init_chunk(&f->chunks[l][i], (float)(i * CHUNK_SIZE), l, height, f->seed);
    }
    f->particle_count = 200; f->particles = (Particle*)calloc(f->particle_count, sizeof(Particle));
    return f;
}
...
void draw_ground(VirtualBitmap *bmp, int layer, float camera_x) {
    ...
    for (int x = 0; x < w; x++) {
        ...
        float terrain_noise = simplex_noise2(wx * 0.008f, (float)layer * 50.0f) * 3.0f;
        int gy = (int)(ground_base_y + terrain_noise);
        for (int y = gy; y < h; y++) draw_pixel_ext(bmp, x, y, MAT_GROUND);
        if (simplex_noise2(wx * 0.3f, (float)layer * 10.0f) > 0.85f) draw_pixel_ext(bmp, x, gy - 1, MAT_GROUND);
    }
}

/* Updates tree physics with wind and particle simulation */
static void update_node(Node *n, float dt, float time, int layer, float wind_base) {
    if (n->parent) {
        float height_factor = (1.0f - (float)n->ry / 1000.0f);
        float wind = simplex_noise2(n->rx * 0.005f, time * 0.4f) * wind_base * height_factor;
        n->ax = wind * 15.0f;
        float dx = n->rx - n->x; n->ax += dx * SPRING_FORCE;
        n->vx += n->ax * dt; n->vx *= DAMPING; n->x += n->vx * dt;
    }
    for (int i = 0; i < n->child_count; i++) update_node(n->children[i], dt, time, layer, wind_base);
}
    ...
    for (int l = 0; l < LAYER_COUNT; l++) {
        ...
        for (int i = 0; i < CHUNK_POOL_SIZE; i++) {
            ...
            if (c->world_x + CHUNK_SIZE < view_start - CHUNK_SIZE) {
                ...
                for(int j=0; j<CHUNK_POOL_SIZE; j++) if(f->chunks[l][j].world_x > furthest_x) furthest_x = f->chunks[l][j].world_x;
                free_chunk(c);
                init_chunk(c, furthest_x + CHUNK_SIZE, l, f->height, f->seed);
            }
            if (c->active) for (int t = 0; t < c->tree_count; t++) update_node(c->trees[t].root, dt, time, l, wind_base);
        }
    }
    for (int i = 0; i < f->particle_count; i++) {
        if (!f->particles[i].active) {
            if (rand() % 80 == 0) {
                f->particles[i].active = 1; f->particles[i].layer = rand() % LAYER_COUNT;
                float parallax = (f->particles[i].layer == 0) ? 1.0f : (f->particles[i].layer == 1 ? 0.6f : 0.3f);
                f->particles[i].x = f->camera_x * parallax + (float)(rand() % f->width); f->particles[i].y = -5.0f;
                f->particles[i].vx = (float)(rand() % 10 - 5); f->particles[i].vy = (float)(rand() % 3 + 3);
            }
            continue;
        }
        f->particles[i].vy += 4.0f * dt; f->particles[i].x += f->particles[i].vx * dt; f->particles[i].y += f->particles[i].vy * dt;
        float parallax = (f->particles[i].layer == 0) ? 1.0f : (f->particles[i].layer == 1 ? 0.6f : 0.3f);
        if (f->particles[i].y > f->height || f->particles[i].x < f->camera_x * parallax - 50 || f->particles[i].x > f->camera_x * parallax + f->width + 50) f->particles[i].active = 0;
    }
}
...
void draw_forest(Forest *f, VirtualBitmap **bmps, int layer_count) {
    for (int l = 0; l < layer_count; l++) {
        ...
        for (int i = 0; i < CHUNK_POOL_SIZE; i++) {
            Chunk *c = &f->chunks[l][i];
            if (c->active) for (int t = 0; t < c->tree_count; t++) draw_pine_tree(&c->trees[t], bmps[l], offset_x);
        }
        for (int i = 0; i < f->particle_count; i++) {
            if (f->particles[i].active && f->particles[i].layer == l) {
                int px = (int)(f->particles[i].x - offset_x); int py = (int)f->particles[i].y;
